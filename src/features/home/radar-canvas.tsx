...
export function RadarCanvas({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    // ponytail: re-assign to const after guard — TS loses narrowing in closures
    const canvas = el;
    const context = ctx;
    ...
    function frame(now: number) {
      ...
      context.translate(depthX, depthY);

      // rings
      context.lineWidth = 1;
      for (let r = 1; r <= 4; r++) {
        context.beginPath();
        context.strokeStyle = r === 4 ? gridStrong : grid;
        context.arc(cx, cy, (R * r) / 4, 0, Math.PI * 2);
        context.stroke();
      }
      // cross lines
      ...
      if (pointerActive && !reduced) {
        ...
        context.beginPath();
        context.strokeStyle = `${blue}0.28)`;
        context.lineWidth = 1;
        context.arc(fx, fy, 11 + Math.sin(now / 380) * 2, 0, Math.PI * 2);
        context.stroke();
        context.beginPath();
        context.fillStyle = `${blue}0.65)`;
      ...
      if (!reduced) {
        ...
        for (let i = 0; i < steps; i++) {
          ...
          context.arc(cx, cy, R, a1, a0);
          context.closePath();
          context.fillStyle = `${green}${alpha})`;
          context.fill();
        }
        // leading edge line
        context.beginPath();
        context.strokeStyle = `${green}0.85)`;
        context.lineWidth = 1.5;
        context.moveTo(cx, cy);
        context.lineTo(cx + Math.cos(sweep) * R, cy + Math.sin(sweep) * R);
        context.stroke();
      }
      ...
      for (const b of blips) {
        ...
        if (b.life > 0) {
          ...
          // glow
          context.beginPath();
          context.fillStyle = `${green}${0.25 * fade})`;
          context.arc(bx, by, b.size + 6 * (1 - fade * 0.3), 0, Math.PI * 2);
          context.fill();
          // core
          context.beginPath();
          context.fillStyle = `${green}${fade})`;
          context.arc(bx, by, b.size, 0, Math.PI * 2);
          context.fill();
          // ping ring
          if (!reduced) {
            context.beginPath();
            context.strokeStyle = `${green}${0.5 * fade})`;
            context.lineWidth = 1;
            context.arc(bx, by, b.size + 14 * t, 0, Math.PI * 2);
            context.stroke();
          }
        }
      }
      ...
      if (burst > 0) {
        context.beginPath();
        context.strokeStyle = `${blue}${burst * 0.5})`;
        context.lineWidth = 1.25;
        context.arc(cx, cy, R * (1 - burst * 0.82), 0, Math.PI * 2);
        context.stroke();
      }

      // center dot
      context.beginPath();
      context.fillStyle = `${blue}0.9)`;
      context.arc(cx, cy, 3.5, 0, Math.PI * 2);
      context.fill();
      context.beginPath();
      context.strokeStyle = `${blue}${0.22 + Math.sin(now / 420) * 0.08})`;
      context.arc(cx, cy, 8 + Math.sin(now / 420) * 1.5, 0, Math.PI * 2);
      context.stroke();
      context.restore();
